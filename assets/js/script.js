...
function enabled_button() {
  ...
  if (selections_count === 3) {
    ...
    enabled_button.removeAttribute("disabled");
    enabled_button.innerHTML = "Fechar pedido";
  } else {
    const enabled_button = document.querySelector(".btn-bottom-bar");

    enabled_button.classList.remove("btn-bottom-bar-custom");
    enabled_button.setAttribute("disabled", "");
    enabled_button.innerHTML = `\
...
function send_request() {
  const title_product = document.querySelectorAll(
    ".selected .card-product-title"
  );
  const phone_number = "";
  ...
  if (enabled_send_request !== null && name !== null && address !== null) {
    ...
    open(
      "https://wa.me/" +
        phone_number +
        "?text=" +
        encodeURIComponent(custom_message)
    );
  }
}
...
function replace_order_items() {
  const confirm_order = document.querySelector(".confirm-order");
  const confirm_order_items = confirm_order.querySelector(
    ".confirm-order-items-name"
  );
...
function replace_order_values() {
  const confirm_order = document.querySelector(".confirm-order");
  const confirm_order_items = confirm_order.querySelector(
    ".confirm-order-items-value"
  );

  const price_product = document.querySelectorAll(
    ".selected .card-product-price"
  );
  ...
  if (price_product[0] !== undefined) {
    price_first_product = parseFloat(
      price_product[0].innerText.replace("R$ ", "").replace(",", ".")
    );
  }

  if (price_product[1] !== undefined) {
    price_second_product = parseFloat(
      price_product[1].innerText.replace("R$ ", "").replace(",", ".")
    );
  }

  if (price_product[2] !== undefined) {
    price_third_product = parseFloat(
      price_product[2].innerText.replace("R$ ", "").replace(",", ".")
    );
  }
  ...
  first_product = confirm_order_items.querySelector(
    ".first-product-item-value"
  );
  first_product.innerHTML = price_first_product_custom;

  second_product = confirm_order_items.querySelector(
    ".second-product-item-value"
  );
  second_product.innerHTML = price_second_product_custom;

  third_product = confirm_order_items.querySelector(
    ".third-product-item-value"
  );
  third_product.innerHTML = price_third_product_custom;

  total_products = confirm_order_items.querySelector(
    ".total-product-items-value"
  );
